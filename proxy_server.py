...
def kill_proxy_process():
    try:
        ...
        subprocess.check_output("ps aux|grep ssserver|awk '{print $2}'|xargs kill -9".format("shadowsock.json"), shell=True)
        subprocess.check_output("ps aux|grep ssserver|awk '{print $2}'|xargs kill -9".format("proxy_server"),shell=True)
        subprocess.check_output("ps aux|grep {}|awk '{print $2}'|xargs kill -9".format("ssserver"),shell=True)
